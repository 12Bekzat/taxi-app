...
const COLORS = {
  red: '#E30613', white: '#FFFFFF', black: '#0F0F10', grey: '#6A6A6A'
};
 ...
 *  title?: string         // заголовок, например "Водительские права"
...
export default function CameraCaptureModal({ visible, onClose, onResult, title='Документ', aspect=1.6 }) {
  ...
  useEffect(() => {
    if (permission?.granted === false && visible) requestPermission();
  }, [permission, visible]);
  ...
  const acceptShot = () => {
    if (side === 'front') {
      ...
      const backUri = preview;
      setPreview(null);
      onResult?.({ frontUri, backUri });
      onClose?.();
    }
  };
    ...
    return (
      <Modal visible transparent animationType="fade" onRequestClose={onClose}>
        ...
        <View style={styles.sheet}>
          ...
          <Text style={styles.text}>Разрешите использование камеры для съёмки документов.</Text>
  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.wrap}>
        ...
        {/* Overlay с рамкой и подсказками */}
        <View pointerEvents="none" style={StyleSheet.absoluteFill}>
          ...
          <View style={styles.bottom}>
            ...
            <Text style={styles.h2}>Держите документ в рамке, без бликов. Текст должен быть читаем.</Text>
          </View>
        </View>
          ...
          {preview ? (
            <>
              <Pressable style={[styles.controlBtn, styles.ghost]} onPress={retake}>
                <Text style={[styles.controlText, { color: COLORS.black }]}>Переснять</Text>
              </Pressable>
              <Pressable style={[styles.controlBtn, { backgroundColor: COLORS.red }]} onPress={acceptShot}>
                <Text style={styles.controlText}>{side === 'front' ? 'Далее' : 'Готово'}</Text>
              </Pressable>
            </>
          ) : (
            <>
              <Pressable style={[styles.controlBtn, styles.ghost]} onPress={onClose}>
                <Text style={[styles.controlText, { color: COLORS.black }]}>Отмена</Text>
              </Pressable>
...
const styles = StyleSheet.create({
  ...
  frame:{ width:'80%', borderRadius:12, borderWidth:2, borderColor:'rgba(255,255,255,0.7)' },
  ...
  sheet:{ position:'absolute', left:24, right:24, top:'35%', backgroundColor:'#FFF', borderRadius:16, padding:16 },
  title:{ fontWeight:'900', fontSize:16, marginBottom:6 },
  text:{ color:COLORS.grey, marginBottom:12 },
  btn:{ height:48, borderRadius:14, alignItems:'center', justifyContent:'center', marginTop:8 },
